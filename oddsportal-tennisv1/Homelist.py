...
from os import listdir, altsep, sep, mkdir
...
class Homelist():
    ...
    def scrape_leagues(self, do_verbose_output=False):
        ...
        if do_verbose_output is True:
            print("\n Done scraping the list of leagues. Start scraping games of each league:")

        for row_league_dict in cols.to_dict(orient='records'):
    ...
    def scrape_gamesofleagues(self, league_dict, do_verbose_output=False):
        ...
        for row in years_tbl_rows:
            ...
            Columslist=["League","country","date","player1","player2","result", "HomeAwayodd01","HomeAwayodd02","AHgames" ,"AHodd01","AHodd02","OUgames","OUodds1","OUodds2"]
            datas={}
            ...
            for id in Columslist:
                cols[id] = datas[id]
            tournament_path=getcwd()+sep++league_dict["league"]+year_name_curr+'data.csv'
            tournamentlistcsv=open(tournament_path,"w")
            cols.to_csv(tournament_path)
            tournamentlistcsv.close()


                # Columslist=["country","country","url"]
                # datas={}
                # datas[Columslist[0]]=league_name
                # datas[Columslist[1]]=league_country
                # datas[Columslist[2]]=league_address
                ...
                # for id in Columslist:
                #     cols[id] = datas[id]
                # # LeagueList_path=getcwd()+sep+"results"+sep+'LeagueList.csv'
                # LeagueList_path = 'LeagueList.csv'
                # lealist=open(LeagueList_path,"w")
                # cols.to_csv(LeagueList_path)
                # lealist.close()
        ...
        if do_verbose_output is True:
            print("Done scraping the tournament list of the league. starting scaping the tournaments details:")
        ...
        for row_ahodds_rows in ahodds_rows: # subwhite tables
            ...
            if len(td_row_ahodds_rows)!=0:
                if ("Pinnacle" == td_row_ahodds_rows[0].find_all(class_='name')[0].string):
                    ...
                    currentdistance=(float(AHodds_1)-2)**2+(float(HAodds_2)-2)**2
