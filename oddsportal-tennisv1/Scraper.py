"""
Tennis match results scraping object.
"""
...
class Scraper():
    ...
    def scrape_url(self, url):
        ...
        # tournament_tbl_soup = BeautifulSoup(tournament_tbl_html, "lxml") # quicker than standard package "html.parser", but need C package
        ...
        for row in significant_rows:
            if self.is_date(row) is True:
                ...
                this_match.set_players(participants)
    ...
    def get_date(self, tag):
        ...
        this_date = tag.find(class_="ficon").string
    ...
    def get_time(self, tag):
        """
        Extract the time from an HTML tag for a tennis match row.
    ...
    def get_participants(self, tag):
        ...
        if len(parsed_strings)==4:
            participants.append(parsed_strings[0])
            participants.append(parsed_strings[1])
            participants.append(parsed_strings[2])
            participants.append(parsed_strings[3])
        else:
            participants.append(parsed_strings[0])
            participants.append(parsed_strings[-1])
        return participants
    ...
    def get_scores(self, tag):
        """
        Extract the scores for each team from an HTML tag for a tennis match
        row.
    ...
    def get_odds(self, tag):
        """
        Extract the betting odds for a match from an HTML tag for a tennis
        match row.
    ...
    def is_invalid_game_from_score_string(self, score_str):
        """
        Assess, from the score string extracted from a tennis match row,
