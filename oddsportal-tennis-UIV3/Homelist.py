...
######################################################新定义的
    ...
    def get_odds(self, game_url):
        ...
        if clicked:
            ah_hides=self.browser.find_elements_by_partial_link_text("Compare odds") # located to the inner link item
            # unfold all the "games" tables
            for row_ah_hides in ah_hides:
                    row_ah_hides.click()
            ...
            table_container_rows = ahodds_soup.select('.table-container')

            # find all the "pinnacle" and conpare
            distance=10000000
            for row_table_container_rows in table_container_rows: # subwhite tables
                ah_games = row_table_container_rows.select('strong')
                if len(ah_games)!=0:
                    if "Games" in ah_games[0].string:
                        ah_hides = row_table_container_rows.select('span')
                        if "Hide " in ah_hides[4].string:
                            lo_row_ahodds_rows = row_table_container_rows.select('.lo')
                            for row_ahodds_rows in lo_row_ahodds_rows: # subwhite tables
        ...
        if clicked:
            ou_hides=self.browser.find_elements_by_partial_link_text("Compare odds") # located to the inner link item
            # unfold all the "games" tables
            for row_ou_hides in ou_hides:
                    row_ou_hides.click()
            ...
            outable_container_rows = ouodds_soup.select('.table-container')

            # find all the "pinnacle" and conpare
            distance=10000000
            for row_outable_container_rows in outable_container_rows: # subwhite tables
                ou_games = row_outable_container_rows.select('strong')
                if len(ou_games)!=0:
                    if "Games" in ou_games[0].string:
                        ou_hides = row_outable_container_rows.select('span')
                        if "Hide " in ou_hides[4].string:
                            lo_row_ouodds_rows = row_outable_container_rows.select('.lo')
                            for row_ouodds_rows in lo_row_ouodds_rows: # subwhite tables
