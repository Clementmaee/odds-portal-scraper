...
class Homelist():
    ...
    def __init__(self, current_set):
        ...
        year_dir=self.currpath+sep+"yearlist"
    ...
    def scrape_gamesofleagues(self, league_dict, do_verbose_output=False, do_scrape_all=True):
        ...
        if do_scrape_all is True:
            ...
            while True:
                try:
                    year_from = int(input("scrape the data from year: "))
                except ValueError:
                    print("input is incorrect, please input again:")
                    continue
                else:
            ...
            while True:
                try:
                    year_end = int(input("scrape the data to year: "))
                except ValueError:
                    print("input is incorrect, please input again:")
                    continue
                else:
                    if year_end >= year_from:
                        ...
                        continue
            output_str = "the years to be scraped is from " + str(year_from)+ " to " + str(year_end)
            print(output_str)
        ...
        if not(os.path.exists(year_dir)):
            os.mkdir(year_dir)
        LeagueyearList_path=year_dir+sep+"year list of "+league_dict["league"]+'.csv'
    ...
    def get_odds(self, game_url):
        ...
        for row_haodds_rows in haodds_rows:
            ...
            if len(td_rows_haodds_rows)!=0:
                ...
                if "Pinnacle" == td_rows_haodds_rows[0].find_all(class_='name')[0].string:
